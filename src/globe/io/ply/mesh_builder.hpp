...
private:
    using Point3 = cgal::Point3;
    using Vector3 = cgal::Vector3;
    ...
    VertexIndex get_or_create_vertex(
        SurfaceMesh &mesh,
        const Point3 &point
    );
    ...
    std::map<Point3, VertexIndex> _vertices_by_point;
};
...
inline void MeshBuilder::build_arc(
    ...
    VertexIndex prev_vertex = get_or_create_vertex(mesh, source);
    Vector3 arc_normal = io::normal(source, target);
    Point3 prev_offset = create_offset_point(source, arc_normal);
    VertexIndex prev_offset_vertex = get_or_create_vertex(mesh, prev_offset);
...
inline void MeshBuilder::add_segment_at_parameter(
    ...
    VertexIndex curr_vertex = get_or_create_vertex(mesh, sampled_point);
    Point3 offset_point = create_offset_point(sampled_point, arc_normal);
    VertexIndex curr_offset_vertex = get_or_create_vertex(mesh, offset_point);
...
inline VertexIndex MeshBuilder::get_or_create_vertex(
    SurfaceMesh &mesh,
    const Point3 &point
) {
    ...
    VertexIndex vertex = mesh.add_vertex(point);
...
inline Point3 MeshBuilder::create_offset_point(const Point3 &point, const Vector3 &arc_normal) const {
    Vector3 radius_vec = cgal::to_vector(point);
