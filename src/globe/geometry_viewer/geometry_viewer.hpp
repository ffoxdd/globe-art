...
GeometryViewer::GeometryViewer(GeometryViewer::Config &&config) :
    CGAL::Basic_viewer_qt(config.parent, "GeometryViewer", true, true, true, false, false) {
    _key_press_callback = config.key_press_callback;
}

GeometryViewer::GeometryViewer(QWidget *parent) :
    GeometryViewer(Config{.parent = parent}) {
}
